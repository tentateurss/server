...
public class StructureManager {
    ...
    private void buildSpawn(Location loc) {
        ...
        // 4) v7: Тренировочные манекены — БОЛЬШЕ (8 шт), разной высоты,
        // с разной «бронёй» (тыква / череп / pumpkin / skull). Расставлены
        // в две линии перед лагерем — площадка для стрельбы и боя.
        int[][] dummyOffsets = {
            {-5, -4}, {-3, -4}, {-1, -4}, { 1, -4},
            { 3, -4}, { 5, -4},
            {-5, -1}, { 5, -1}
        ...
        Material[] heads = {
            Material.CARVED_PUMPKIN, Material.SKELETON_SKULL,
            Material.WITHER_SKELETON_SKULL, Material.ZOMBIE_HEAD,
            Material.PLAYER_HEAD, Material.CARVED_PUMPKIN,
            Material.SKELETON_SKULL, Material.WITHER_SKELETON_SKULL
        };
        for (int i = 0; i < dummyOffsets.length; i++) {
            int[] off = dummyOffsets[i];
            int dxv = off[0], dzv = off[1];
            // 2-блочная «фигура» — корпус из сена / стенка + голова сверху
            world.getBlockAt(cx + dxv, feetY,     cz + dzv).setType(Material.HAY_BLOCK);
            world.getBlockAt(cx + dxv, feetY + 1, cz + dzv).setType(Material.HAY_BLOCK);
            world.getBlockAt(cx + dxv, feetY + 2, cz + dzv).setType(heads[i % heads.length]);
        }
        // Бронированный «чемпион» — стек из 3 блоков с шлемом из обсидиана.
        int champX = cx, champZ = cz - 5;
        world.getBlockAt(champX, feetY,     champZ).setType(Material.POLISHED_BLACKSTONE);
        world.getBlockAt(champX, feetY + 1, champZ).setType(Material.HAY_BLOCK);
        world.getBlockAt(champX, feetY + 2, champZ).setType(Material.HAY_BLOCK);
        world.getBlockAt(champX, feetY + 3, champZ).setType(Material.WITHER_SKELETON_SKULL);
        // Стрелковая мишень
        int targetX = cx + 7, targetZ = cz - 4;
        world.getBlockAt(targetX, feetY + 1, targetZ).setType(Material.RED_WOOL);
        world.getBlockAt(targetX, feetY + 2, targetZ).setType(Material.WHITE_WOOL);
        world.getBlockAt(targetX, feetY + 3, targetZ).setType(Material.RED_WOOL);
        // Щит-мишень слева
        int shieldX = cx - 7, shieldZ = cz - 4;
        world.getBlockAt(shieldX, feetY + 1, shieldZ).setType(Material.IRON_BLOCK);
        world.getBlockAt(shieldX, feetY + 2, shieldZ).setType(Material.IRON_BARS);
        ...
        // 11) Указатель — v7: убрана деревянная табличка, текст вынесен
        // в TextDisplay-голограмму (см. BeachGenerator.postProcessSigns).
