    ...
    private static final class BlockOp {
        ...
        void apply(World world) {
            ...
            if (data != null) {
                ...
                // Если это листья — сделать persistent, чтобы не decay-ить.
                if (data instanceof Leaves) {
                    Leaves cur = (Leaves) b.getBlockData();
                    if (!cur.isPersistent()) {
                        cur.setPersistent(true);
                        b.setBlockData(cur, false);
                    }
                }
            } else if (mat != null) {
                Material m = mat.get();
                if (m != null && b.getType() != m) {
                    b.setType(m, false);
                    // Авто-persistent для всех листьев — чтобы декорации
                    // не «разваливались» со временем (фидбэк v3).
